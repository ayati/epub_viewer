...
`spineCount` is stored in the **value** (not derivable from the key in the new format). `buildReadingList()` and `markAsFinished()` prefer `val.spineCount` and fall back to `parsed.spineCount` (legacy keys only). `saveBookMeta()` writes the current `state.spine.length` as `spineCount` on every open, so once a user re-opens the new (longer) file, the displayed chapter count updates automatically.
